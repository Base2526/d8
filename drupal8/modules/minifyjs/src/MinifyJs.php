...
class MinifyJs {
  ...
  public function minifyFile($fid, $reset = FALSE) {
    ...
    $tmp_file = $this->fileSystem->getTempDirectory() . '/' . $file_name;
    $file_uri = $minifyjs_folder . '/' . $file_name;
    if (file_put_contents($tmp_file, $minified) !== FALSE) {
      ...
      return t('Could not save the file - %file', ['%file' => $this->fileSystem->getTempDirectory() . '/' . $file_name]);
    }
  }
